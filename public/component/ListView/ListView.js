...
class ListView extends React.Component{
    ...
    UNSAFE_componentWillMount(){
        let current = this;
        //맨 처음 값을 가지고 옵니다.
        AsyncStorage.getItem(StorageName).then((res,rej) => {
            if(res !== null ){
    ...
    loadData = async(title) => {
        ...
        try{
            ...
            if(oldData === null){
                ...
                let isOld = dataArr.some((item) => {         //중복 확인
                    return item === title;
                })
        ...
        return (
            <View style={styles.container}>
                <FlatList
                    ...
                    renderItem={({ item }) =>
                        <TouchableOpacity onPress={() => this.onClick(item.url, item.title)}>
                            <ListItem
                                title={item.title}
                                des={item.des}
                    ...
                    onEndReached={() => { this.props._getData(); }}
